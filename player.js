...
export class Player {
     ...
     * @param {number} [jumpHeight=15] Vertical force applied when jump() fires
     */
    constructor(tileX=0, tileY=0, maxSpeed=4, jumpHeight=15) {
        /**
         ...
         * this.relativePosition = { x: 10, y: 10 };
         * @type {{ x: 0, y: 0 }}
        ...
        this.jumpForce = 0;
        /**
         * Time in which the player is not able to jump since the last jump
         * @type {number}
         * @readonly
         */
    ...
    draw(level) {
        ...
        if (this.timeSinceLastJump == 0) {
            ...
            if (this.walkFrameCount >= this.WALK_ANIM_SPEED-(this.momentumHorizontal/2)) {
     ...
     */
    gravity() {
        // deduct jumpForce so an reverse parabula is reached
    ...
    /**
     * Jump if no horizontal momentum and jump timeout reached
     */
    jump() {
        if (this.momentumVertical <= this.GRAVITY && this.timeSinceLastJump >= this.JUMP_TIMEOUT) this.jumpForce = this.JUMP_HEIGHT;
    }
    /**
